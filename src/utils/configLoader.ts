...
import { AppConfig, ResolvedLLMConfig, GITAI_COMMAND } from '../types/index.js';
...
const defaultLoaders = {
    '.yaml': (filepath: string, content: string) => yaml.load(content) as any, // any for now
    '.yml': (filepath: string, content: string) => yaml.load(content) as any,
};

// For searching in specific directory
function createExplorer(searchFrom?: string) {
...
async function loadTextFile(filePath: string): Promise<string | undefined> {
    try {
...
export async function getConfig(): Promise<AppConfig> {
    ...
    let finalConfig = merge({}, defaultConfig) as AppConfig;
 ...
 */
export function getResolvedLLMConfig(config: AppConfig, command?: GITAI_COMMAND): ResolvedLLMConfig {
 ...
 */
export async function getConfigRaw(): Promise<{ path: string; raw: unknown }> {
