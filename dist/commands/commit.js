...
import { GITAI_COMMAND } from '../types/index.js';
import { logger } from '../utils/logger.js';
import inquirer from 'inquirer';
import ora from 'ora';
import chalk from 'chalk';
export async function handleCommitCommand(options, config) {
    try {
        ...
        if (options.printPrompt) {
            const fullPrompt = await buildCommitPrompt(diff, options.prompt, K, config, GITAI_COMMAND.COMMIT);
            if (!fullPrompt) {
        ...
        const commitSuggestions = await generateCommitMessagesAI(diff, options.prompt, K, config, GITAI_COMMAND.COMMIT);
        // Clear the timer
        clearInterval(timer);
        ...
        if (selectedMessageValue && selectedMessageValue !== 'skip' && selectedMessageValue !== 'edit') {
            const finalMessage = selectedMessageValue;
